...
def cosine_similarity(arr1, arr2):
    cs = nd.dot(nd.array(arr1), nd.array(arr2)) / (nd.norm(nd.array(arr1)) + 1e-9) / (nd.norm(nd.array(arr2)) + 1e-9)
    return cs.asnumpy()[0]
...
def dot(arr1, arr2):
    cs = nd.dot(nd.array(arr1), nd.array(arr2))
    return cs.asnumpy()[0]
...
def norm(arr):
    # return torch.norm(arr).asnumpy()[0]
    return nd.linalg.norm(nd.array(arr),ord = 2, keepdims=True)[0]
